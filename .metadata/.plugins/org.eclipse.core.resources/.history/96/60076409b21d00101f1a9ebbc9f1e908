...
//	}
    ...
    public ResponseEntity<CollectionModel<EntityModel<Member>>> getAllMembers() {
        ...
        return ResponseEntity.ok(CollectionModel.of(members, linkTo(methodOn(MemberController.class)
	...
	@PatchMapping("/{id}")
	public ResponseEntity<Member> patchMember(@PathVariable Long id, 
			@RequestBody Member memberDetails) {
	    // apply partial updates to member with id
		Optional<Member> patchMember = memberService.patchMember(id, memberDetails);
		return patchMember
				.map(ResponseEntity::ok)
				.orElseGet(()->ResponseEntity.notFound().build());
